...
class MeldStudioInstance extends InstanceBase {
				...
				try {
					new QWebChannel(transport, (channel) => {
						...
						if (this.qweb?.isRecordingChanged?.connect) {
							this.qweb.isRecordingChanged.connect(() => this._onRecordingStateChange(!!this.qweb.isRecording))
						}
						if (this.qweb?.isStreamingChanged?.connect) {
							this.qweb.isStreamingChanged.connect(() => this._onStreamingStateChange(!!this.qweb.isStreaming))
						}
	...
	_refreshFeedbackChoices() {
		const sceneChoices = Object.values(this.scenes).map((s) => ({
			id: s.id,
			label: s.name,
		}))
		this.setFeedbackDefinitions({
